...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (this.addedToCart) {
      ...
      const response = confirm('You Sure?');
      next(response);
    }
  },
  ...
  computed: {
    ...
    headBorderStyle() {
      return { border: this.selectedRobot.head.onSale ? '3px solid red' : '3px solid white' };
    },
  ...
  methods: {
    addToCart() {
      ...
      this.$store.dispatch('addRobotToCart', { ...robot, cost }).then(() => this.$router.push('/cart'));
